...
Public Function SDO(ByVal N As Integer) As Integer
...
Public Function ValidarLoginMSG(ByVal numero As Integer) As Integer
    ...
    Dim AuxInteger3 As Integer
    
    AuxInteger = SumaDigitos(numero)
    AuxInteger2 = SumaDigitosMenos(numero)
    AuxInteger3 = SDO(numero)
    ValidarLoginMSG = Complex(AuxInteger + AuxInteger2 + AuxInteger3) - 7
End Function
...
Function AsciiValidosPass(ByVal cad As String) As Boolean
    Dim car As Byte
    Dim I As Long
    ...
    For I = 1 To Len(cad)
        ...
        If (car < 48 Or car > 57) And ((car < 97 Or car > 122) Or car = Asc("º")) And (car <> 255) And (car <> 32) Then
            Exit Function
        End If
    Next I
    
    AsciiValidosPass = True
End Function

Public Function NombreAccountCheck() As Boolean

Dim loopc As Integer
Dim CharAscii As Integer

For loopc = 1 To Len(cuentaNom)
    CharAscii = Asc(mid$(cuentaNom, loopc, 1))
    If Not LegalCharacter(CharAscii) Or Not AsciiValidos(Chr$(CharAscii)) Then
        MsgBox ("Nombre inválido. El caractér " & Chr$(CharAscii) & " no está permitido.")
        NombreAccountCheck = False
        Exit Function
    End If
Next loopc

For loopc = 1 To Len(cuentaPass)
    CharAscii = Asc(mid$(cuentaPass, loopc, 1))
    If Not AsciiValidosPass(Chr$(CharAscii)) Then
...
If Len(cuentaNom) > 15 Then
    MsgBox ("El nombre debe tener menos de 15 letras.")
    NombreAccountCheck = False
    Exit Function
End If

If Len(cuentaPass) > 15 Then
    MsgBox ("La contraseña debe tener menos de 15 letras.")
    NombreAccountCheck = False
    Exit Function
End If

If Not CheckMailString(cuentaEmail) Then
    MsgBox ("Debe ingresar un email válido.")
    NombreAccountCheck = False
    Exit Function
End If

NombreAccountCheck = True
...
Public Function LegalCharacter(ByVal KeyAscii As Integer) As Boolean
'*****************************************************************
...
'*****************************************************************
    ...
    If KeyAscii = 34 Or KeyAscii = 33 Or KeyAscii = 42 Or KeyAscii = 47 Or KeyAscii = 58 Or KeyAscii = 60 Or KeyAscii = 62 Or KeyAscii = 63 Or KeyAscii = 92 Or KeyAscii = 124 Then
        Exit Function
    End If
...
On Error Resume Next
    ...
    splitting = Split(Lst, "ipp")(1)
    
    'ServersLst(1).Ip = ReadField(1, splitting, 44)
    'ServersLst(1).Puerto = ReadField(2, splitting, 44)
    
    'frmConnect.IPTxt.Text = ReadField(1, splitting, 44)
   ' frmConnect.PortTxt.Text = ReadField(2, splitting, 44)
   
   ServerEAO = CStr(ReadField(1, splitting, 44))
   PuertoEAO = CInt(ReadField(2, splitting, 44))
   
   'MsgBox ServerEAO & PuertoEAO
...
Public Function CurServerPort() As Integer
    If CurServer <> 0 Then
        CurServerPort = PuertoEAO
    Else
        CurServerPort = frmConnect.PortTxt
    End If
End Function
...
#Else
    MD5HushYo = Trim(MD5String(App.EXEName & ".exe"))
...
#End If
    ...
    If Not FileExist(App.Path & "\Init\Cabezas.ind", vbArchive) Then
        Call MsgBox("ERROR: faltan archivos criticos para poder continuar.", vbCritical + vbOKOnly)
        End
    End If
    ...
    AddtoRichTextBox frmCargando.status, "Buscando servidores....", 0, 128, 128, 0, 0, 1
    
    'ServerGH = GetVar(App.Path & "\MSWSK32.OCX", "INIT", "ip")
    
   ' If Not InStr(ServerGH, "200.43.") Then
      '  frmMain.Inet1.url = "http://www.empiresao.com.ar/s/server.txt"
      '  ServerGH = frmMain.Inet1.OpenURL
      '  If ServerGH = "" Then End
      '  ServerGH = Split(ServerGH, "s=")(1)
      '  ServerGH = Trim(ServerGH)
     '   Call WriteVar(App.Path & "\MSWSK32.OCX", "INIT", "ip", ServerGH)
    'End If
    ...
    'frmCargando.Label1.Caption = "Buscando servidores....."
...
'TODO : esto de ServerRecibidos no se podría sacar???
    ...
    AddtoRichTextBox frmCargando.status, "Iniciando constantes...", 0, 128, 128, 0, 0, 1
  ...
  '  frmOldPersonaje.NameTxt.Text = Config_Inicio.Name
   ' frmOldPersonaje.PasswordTxt.Text = ""
    ...
    AddtoRichTextBox frmCargando.status, "Cargando Sonidos....", 0, 128, 128, 0, 0, 1
    AddtoRichTextBox frmCargando.status, "Hecho", , 128, 128, 1

Dim loopc As Integer

LastTime = GetTickCount

    Call InitTileEngine(frmMain.hwnd, 145, 11, 32, 32, 13, 17, 9)
    
    Call AddtoRichTextBox(frmCargando.status, "Creando animaciones extra....", 0, 128, 128, 0, 0, 0)
...
UserMap = 1
    ...
    If FileExist(App.Path & "\Init\opts.eao", vbNormal) = False Then
        MSNshow = 1
        Call WriteVar(App.Path & "\Init\opts.eao", "INIT", "Active", "1")
    Else
        MSNshow = GetVar(App.Path & "\Init\opts.eao", "INIT", "Active")
    End If
    
    mF1 = GetVar(App.Path & "\Init\teclas.eao", "INIT", "f1")
    mF2 = GetVar(App.Path & "\Init\teclas.eao", "INIT", "f2")
    mF3 = GetVar(App.Path & "\Init\teclas.eao", "INIT", "f3")
    mF6 = GetVar(App.Path & "\Init\teclas.eao", "INIT", "f6")
    mF7 = GetVar(App.Path & "\Init\teclas.eao", "INIT", "f7")
    mF8 = GetVar(App.Path & "\Init\teclas.eao", "INIT", "f8")
    mF9 = GetVar(App.Path & "\Init\teclas.eao", "INIT", "f9")
    
    'AddtoRichTextBox frmCargando.status, ""
    'AddtoRichTextBox frmCargando.status, "Conectando al servidor MOD...", 0, 128, 128, 0, 0, 0
...
#End If
    ...
    AddtoRichTextBox frmCargando.status, "                    ¡Bienvenido a Argentum Online!", , 128, 128, 1
    
    Unload frmCargando
    
    'Inicializamos el sonido
    Call AddtoRichTextBox(frmCargando.status, "Iniciando DirectSound....", 0, 128, 128, 0, 0, True)
...
'TODO : Porque el pausado de 20 ms???
        ...
        If userMontando = True Then FramesCaballo = 48
        If userMontando = False Then FramesCaballo = 56
        
        'Limitamos los FPS a 18 (con el nuevo engine 60 es un número mucho mejor)
        While (GetTickCount - lFrameTimer) \ FramesCaballo < FramesPerSecCounter
            Sleep 5
        Wend
...
#End If
    ...
    EngineRun = False
    frmCargando.Show
    AddtoRichTextBox frmCargando.status, "Liberando recursos...", 0, 128, 128, 0, 0, 1
    LiberarObjetosDX
