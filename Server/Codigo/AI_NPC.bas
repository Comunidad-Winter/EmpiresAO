...
For Y = Npclist(NpcIndex).Pos.Y - RANGO_VISION_Y To Npclist(NpcIndex).Pos.Y + RANGO_VISION_Y
    For X = Npclist(NpcIndex).Pos.X - RANGO_VISION_X To Npclist(NpcIndex).Pos.X + RANGO_VISION_X
        If X >= MinXBorder And X <= MaxXBorder And Y >= MinYBorder And Y <= MaxYBorder Then
           ...
           If UI > 0 Then
                If Not Criminal(UI) Then
                   If UserList(UI).flags.Muerto = 0 And UserList(UI).flags.Invisible = 0 And UserList(UI).flags.Privilegios = PlayerType.User And UserList(UI).flags.Oculto = 0 Then
                        If Npclist(NpcIndex).flags.LanzaSpells > 0 Then
                              Call NpcLanzaUnSpell(NpcIndex, UI)
                        End If
                        tHeading = FindDirection(Npclist(NpcIndex).Pos, UserList(MapData(Npclist(NpcIndex).Pos.Map, X, Y).UserIndex).Pos)
...
Private Sub SacerdoteCuraVe(ByVal NpcIndex As Integer)
Dim UI As Integer
Dim nPos As WorldPos
Dim headingloop As Byte
Dim tHeading As Byte
Dim Y As Integer
Dim X As Integer
For Y = Npclist(NpcIndex).Pos.Y - (RANGO_VISION_Y - 3) To Npclist(NpcIndex).Pos.Y + (RANGO_VISION_Y - 3)
    For X = Npclist(NpcIndex).Pos.X - (RANGO_VISION_X - 1) To Npclist(NpcIndex).Pos.X + (RANGO_VISION_X - 1)
        If X >= MinXBorder And X <= MaxXBorder And Y >= MinYBorder And Y <= MaxYBorder Then
           UI = MapData(Npclist(NpcIndex).Pos.Map, X, Y).UserIndex
           If UI > 0 Then
                
                If UserList(UI).flags.Invisible = 0 And UserList(UI).flags.Oculto = 0 And UserList(UI).flags.Privilegios = PlayerType.User Then
                
                If UserList(UI).flags.Muerto = 1 Then
                    Call RevivirUsuario(UI)
                    Call SendData(SendTarget.ToIndex, UI, 0, "PRE49")
                End If
                
                If UserList(UI).Pet.MinHP < UserList(UI).Pet.MaxHP Then
                    UserList(UI).Pet.MinHP = UserList(UI).Pet.MaxHP
                    Call SendData(SendTarget.ToIndex, UI, 0, "PRE48")
                End If
                ...
                If UserList(UI).Stats.MinHP < UserList(UI).Stats.MaxHP Then
                    ...
                    Call SendData(SendTarget.ToIndex, UI, 0, "ASH" & UserList(UI).Stats.MinHP)
                    Call SendData(SendTarget.ToIndex, UI, 0, "PRE49")
                End If
        ...
        If X >= MinXBorder And X <= MaxXBorder And Y >= MinYBorder And Y <= MaxYBorder Then
            If Npclist(NpcIndex).Target = 0 And Npclist(NpcIndex).TargetNPC = 0 Then
                ...
                If UI > 0 Then
                   ...
                   And Distancia(Npclist(NpcIndex).Pos, UserList(UI).Pos) > 4 Then
                        tHeading = FindDirection(Npclist(NpcIndex).Pos, UserList(MapData(Npclist(NpcIndex).Pos.Map, X, Y).UserIndex).Pos)
                        Call MoveNPCChar(NpcIndex, tHeading)
                        Exit Sub
                   End If
...
On Error GoTo ErrorHandler
        ...
        Select Case Npclist(NpcIndex).Movement
            ...
            Case TipoAI.GuardiasAtacanCriminales
                Call PersigueCriminal(NpcIndex)
            Case TipoAI.SacerdoteCura
                Call SacerdoteCuraVe(NpcIndex)
         ...
         If X > MinXBorder And X < MaxXBorder And Y > MinYBorder And Y < MaxYBorder Then
             ...
             If MapData(Npclist(NpcIndex).Pos.Map, X, Y).UserIndex > 0 Then
                 'Move towards user
                  ...
                  If UserList(tmpUserIndex).flags.Privilegios = PlayerType.User And UserList(tmpUserIndex).flags.Muerto = 0 And UserList(tmpUserIndex).flags.Invisible = 0 And UserList(tmpUserIndex).flags.Oculto = 0 Then
...
If UserList(UserIndex).flags.Invisible = 1 Or UserList(UserIndex).flags.Oculto = 1 And Npclist(NpcIndex).VeInvis = 0 Then Exit Sub
