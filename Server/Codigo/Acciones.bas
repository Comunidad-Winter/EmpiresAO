...
If UserIndex = 0 Then
    ...
    Do While EncontroIdeal = False And LoopC < 500
        ...
        If UserList(UserIndex).flags.Privilegios = PlayerType.User And _
           UserList(UserIndex).flags.UserLogged = True And UserList(UserIndex).flags.Muerto = 0 Then
...
If UserIndex > 0 Then
    ...
    End If
    GranPoder = UserIndex
    Call SendData(SendTarget.ToAll, 0, 0, "PRE7," & UserList(UserIndex).name & "," & UserList(UserIndex).Pos.Map)
End If
End Sub
