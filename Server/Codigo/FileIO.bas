...
For Hechizo = 1 To NumeroHechizos
    ...
    Hechizos(Hechizo).SoloClase = Leer.GetValue("Hechizo" & Hechizo, "SoloClase")
...
For Object = 1 To NumObjDatas
   ...
   ' If val(Leer.GetValue("OBJ" & Object, "Weight")) = 0 Then
    '    ObjData(Object).oWeight = DefaultWeight
   ' Else
   '     ObjData(Object).oWeight = Leer.GetValue("OBJ" & Object, "Weight")
        'Debug.Print Leer.GetValue("OBJ" & Object, "Weight")
  ' End If
    ...
    ObjData(Object).NoSeCae = val(Leer.GetValue("OBJ" & Object, "Arnocae"))
...
UserList(UserIndex).flags.PuedeAprender = CByte(UserFile.GetValue("FLAGS", "PuedeAprender"))
...
'UserList(UserIndex).Invent.TotalWeight = UserFile.GetValue("Inventory", "TotalWeight")
'UserList(UserIndex).Invent.Weight = UserFile.GetValue("Inventory", "Weight")
...
UserList(UserIndex).Pet.Nombre = UserFile.GetValue("PET", "Nombre")
UserList(UserIndex).Pet.Tipo = CInt(UserFile.GetValue("PET", "Tipo"))
UserList(UserIndex).Pet.ELU = CLng(UserFile.GetValue("PET", "ELU"))
UserList(UserIndex).Pet.ELV = CLng(UserFile.GetValue("PET", "ELV"))
UserList(UserIndex).Pet.Exp = CDbl(UserFile.GetValue("PET", "EXP"))
UserList(UserIndex).Pet.Defensa = CInt(UserFile.GetValue("PET", "Defensa"))
UserList(UserIndex).Pet.MaxHIT = CInt(UserFile.GetValue("PET", "MaxHIT"))
UserList(UserIndex).Pet.MinHIT = CInt(UserFile.GetValue("PET", "MinHIT"))
UserList(UserIndex).Pet.MaxHP = CInt(UserFile.GetValue("PET", "MaxHP"))
UserList(UserIndex).Pet.MinHP = CInt(UserFile.GetValue("PET", "MinHP"))
...
ServerNombre = GetVar(IniPath & "Server.ini", "INIT", "ServerNombre")
ServerWeb = GetVar(IniPath & "Server.ini", "INIT", "ServerWeb")
If ServerNombre = "" Or InStr(ServerNombre, ")") Or InStr(ServerNombre, "(") Or InStr(ServerNombre, ",") Then ServerNombre = "EAOMod Game Server"
...
AvisosTodos = GetVar(IniPath & "Server.ini", "INIT", "AvisosT")
AvisosClan = GetVar(IniPath & "Server.ini", "INIT", "AvisosC")

SMTPServer = GetVar(IniPath & "Server.ini", "INIT", "SMTP")
MailDomain = GetVar(IniPath & "Server.ini", "INIT", "MailDomain")
UserSMTP = GetVar(IniPath & "Server.ini", "INIT", "UserSMTP")
PassSMTP = GetVar(IniPath & "Server.ini", "INIT", "PassSMTP")
...
'DefaultWeight = GetVar(IniPath & "Server.ini", "INIT", "DefaultWeight")

MultiplicadorGP = GetVar(IniPath & "Server.ini", "INIT", "GPUser")
MultiplicadorGPN = GetVar(IniPath & "Server.ini", "INIT", "GPNpc")

Debug.Print MultiplicadorGP & " para users."
Debug.Print MultiplicadorGPN & " para npcs."
...
Debug.Print IntervaloUserPuedePotear
'FrmInterv.txtPuedeAtacar.Text = IntervaloUserPuedeAtacar
...
Hobitton.Map = GetVar(DatPath & "Ciudades.dat", "Hobitton", "Mapa")
Hobitton.X = GetVar(DatPath & "Ciudades.dat", "Hobitton", "X")
Hobitton.Y = GetVar(DatPath & "Ciudades.dat", "Hobitton", "Y")
...
Call WriteVar(UserFile, "FLAGS", "PuedeAprender", CStr(UserList(UserIndex).flags.PuedeAprender))
...
Call WriteVar(UserFile, "INIT", "Cuenta", CStr(UserList(UserIndex).Accounted))
...
'Call WriteVar(UserFile, "Inventory", "TotalWeight", CDbl(UserList(UserIndex).Invent.TotalWeight))
'Call WriteVar(UserFile, "Inventory", "Weight", CDbl(UserList(UserIndex).Invent.Weight))
...
Call WriteVar(UserFile, "PET", "Nombre", UserList(UserIndex).Pet.Nombre)
Call WriteVar(UserFile, "PET", "Tipo", UserList(UserIndex).Pet.Tipo)
Call WriteVar(UserFile, "PET", "MinHP", UserList(UserIndex).Pet.MinHP)
Call WriteVar(UserFile, "PET", "MaxHP", UserList(UserIndex).Pet.MaxHP)
Call WriteVar(UserFile, "PET", "ELU", UserList(UserIndex).Pet.ELU)
Call WriteVar(UserFile, "PET", "ELV", UserList(UserIndex).Pet.ELV)
Call WriteVar(UserFile, "PET", "EXP", UserList(UserIndex).Pet.Exp)
Call WriteVar(UserFile, "PET", "MaxHIT", UserList(UserIndex).Pet.MaxHIT)
Call WriteVar(UserFile, "PET", "MinHIT", UserList(UserIndex).Pet.MinHIT)
Call WriteVar(UserFile, "PET", "Defensa", UserList(UserIndex).Pet.Defensa)
...
Call WriteVar(npcfile, "NPC" & NpcNumero, "Comercia", val(Npclist(NpcIndex).HechizoClase))
...
Call WriteVar(npcfile, "NPC" & NpcNumero, "Sagrado", val(Npclist(NpcIndex).flags.Sagrado))
...
Npclist(NpcIndex).HechizoClase = val(GetVar(npcfile, "NPC" & NpcNumber, "HechizoClase"))
Npclist(NpcIndex).Subasta = val(GetVar(npcfile, "NPC" & NpcNumber, "Subasta"))
Npclist(NpcIndex).Duelos = val(GetVar(npcfile, "NPC" & NpcNumber, "Duelos"))
...
Npclist(NpcIndex).flags.Sagrado = val(GetVar(npcfile, "NPC" & NpcNumber, "Sagrado"))
...
Sub LogCheat(ByVal BannedIndex As Integer, ByVal motivo As String)

On Error GoTo errhandler

Dim nfile As Integer
nfile = FreeFile ' obtenemos un canal
Open App.Path & "\logs\GenteCheating.log" For Append Shared As #nfile
Print #nfile, UserList(BannedIndex).Accounted & " :" & motivo
Close #nfile
