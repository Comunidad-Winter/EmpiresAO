...
If UserList(UserIndex).Pos.Map = 37 Then
    ...
    Select Case UCase$(UserList(UserIndex).Hogar)
        Case "HOBITTON" 'Vamos a tener que ir por todo el desierto... uff!
            DeDonde = Hobitton
        Case "ULLATHORPE"
            DeDonde = Ullathorpe
        Case "BANDERBILL"
...
'UserList(UserIndex).Invent.Weight = 0
'UserList(UserIndex).Invent.TotalWeight = 0
...
'mmObj = UserList(UserIndex).Invent.Object(Slot).Amount * ObjData(UserList(UserIndex).Invent.Object(Slot).ObjIndex).oWeight

'UserList(UserIndex).Invent.Weight = UserList(UserIndex).Invent.Weight - mmObj
...
If MapData(Map, X, Y).OBJInfo.Amount <= 0 Then
    ...
    If sndRoute = SendTarget.ToMap Then
        Call SendToAreaByPos(Map, X, Y, "ç" & Encrypt_PRO(Crypted, CRYPTO_PASS)) '"BO" & X & "," & Y)
   Else
        Call SendData(sndRoute, sndIndex, sndMap, "ç" & Encrypt_PRO(Crypted, CRYPTO_PASS)) '"BO" & X & "," & Y)
    End If
End If
...
If Obj.ObjIndex > 0 And Obj.ObjIndex <= UBound(ObjData) Then
    ...
    Else
        ...
        Crypted = "HO" & ObjData(Obj.ObjIndex).GrhIndex & "," & X & "," & Y
        
        If sndRoute = SendTarget.ToMap Then
            Call ModAreas.SendToAreaByPos(Map, X, Y, "ç" & Encrypt_PRO(Crypted, CRYPTO_PASS)) '"HO" & ObjData(Obj.ObjIndex).GrhIndex & "," & X & "," & Y)
        Else
            Call SendData(sndRoute, sndIndex, sndMap, "ç" & Encrypt_PRO(Crypted, CRYPTO_PASS)) '"HO" & ObjData(Obj.ObjIndex).GrhIndex & "," & X & "," & Y)
        End If
    End If
End If
...
''PlusWeight = ObjData(MiObj.ObjIndex).oWeight * MiObj.Amount
'PlusWeight = PlusWeight + UserList(UserIndex).Invent.Weight

'If PlusWeight > UserList(UserIndex).Invent.TotalWeight Then
  '  Call SendData(SendTarget.ToIndex, UserIndex, 0, "||No puede cargando mas objetos debido al sobrepeso." & FONTTYPE_FIGHT)
  '  Exit Function
'End If
...
If MapData(UserList(UserIndex).Pos.Map, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).OBJInfo.ObjIndex > 0 Then
    ...
    If ObjData(MapData(UserList(UserIndex).Pos.Map, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).OBJInfo.ObjIndex).Agarrable <> 1 Then
        ...
        If Not MeterItemEnInventario(UserIndex, MiObj) Then
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB69")
        Else
        
            'nnObj = MiObj.Amount * ObjData(MiObj.ObjIndex).oWeight
            
            'Debug.Print "<<<<<<<<<<" & nnObj & ">>>>>>>>>>>>>>>>"
            'Debug.Print "<<<<<<<<<<" & UserList(UserIndex).Invent.Weight & ">>>>>>>>>>>>>>>>>>>"
            
            'UserList(UserIndex).Invent.Weight = UserList(UserIndex).Invent.Weight + nnObj
...
Select Case Obj.OBJType
    ...
    Case eOBJType.otMontura
        ...
        UserList(UserIndex).Char.Body = UserList(UserIndex).Char.exBody
        UserList(UserIndex).Invent.CaballoEqpObjIndex = 0
        UserList(UserIndex).Invent.CaballoEqpSlot = 0
        UserList(UserIndex).Invent.Object(Slot).Equipped = 0
        Call ChangeUserChar(SendTarget.ToMap, 0, UserList(UserIndex).Pos.Map, UserIndex, UserList(UserIndex).Char.Body, UserList(UserIndex).Char.Head, UserList(UserIndex).Char.Heading, UserList(UserIndex).Char.WeaponAnim, UserList(UserIndex).Char.ShieldAnim, UserList(UserIndex).Char.CascoAnim)
...
If Obj.Newbie = 1 And Not EsNewbie(UserIndex) Then
     Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB70")
     Exit Sub
End If
...
Select Case Obj.OBJType
    Case eOBJType.otWeapon
       ...
       Else
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB71")
       End If
    ...
    Case eOBJType.otMontura
        If UserList(UserIndex).Stats.UserSkills(eSkill.Domar) > 49 Then
            ...
            If UserList(UserIndex).Invent.CaballoEqpObjIndex > 0 Then
                Call Desequipar(UserIndex, UserList(UserIndex).Invent.CaballoEqpSlot)
            End If
            
            UserList(UserIndex).Char.exBody = UserList(UserIndex).Char.Body
            
            UserList(UserIndex).Char.Body = Obj.Caballo
            
            UserList(UserIndex).flags.Montado = 1
            
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "MONTA1")
            
            Call ChangeUserChar(SendTarget.ToMap, 0, UserList(UserIndex).Pos.Map, UserIndex, UserList(UserIndex).Char.Body, UserList(UserIndex).Char.Head, UserList(UserIndex).Char.Heading, UserList(UserIndex).Char.WeaponAnim, UserList(UserIndex).Char.ShieldAnim, UserList(UserIndex).Char.CascoAnim)
            
            UserList(UserIndex).Invent.Object(Slot).Equipped = 1
            UserList(UserIndex).Invent.CaballoEqpObjIndex = ObjIndex
            UserList(UserIndex).Invent.CaballoEqpSlot = Slot
            
        Else
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRE35")
       End If
    ...
    Case eOBJType.otArmadura
        ...
        If ClasePuedeUsarItem(UserIndex, UserList(UserIndex).Invent.Object(Slot).ObjIndex) And _
           ...
           'Si esta equipado lo quita
            ...
            Else
                If UserList(UserIndex).flags.Montado = 1 Then
                    UserList(UserIndex).Char.exBody = Obj.Ropaje
                Else
            ...
            Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "TW1")
        ...
        Else
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB72")
        End If
...
Select Case Obj.OBJType
    ...
    Case eOBJType.otGuita
        ...
        Call UpdateUserInv(False, UserIndex, Slot)
        'Call SendUserStatsBox(UserIndex)
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "ASG" & UserList(UserIndex).Stats.GLD)
    ...
    Case eOBJType.otPociones
        ...
        If Not IntervaloPermiteAtacar(UserIndex, False) Then
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB68")
            Exit Sub
        End If
        
        If Not IntervaloPotear(UserIndex) Then Exit Sub
    ...
    'Verifica si esta aproximado al agua antes de permitirle navegar
        ...
        Else
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB73")
        End If
