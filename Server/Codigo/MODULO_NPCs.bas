...
On Error GoTo errhandler
   ...
   MiNPC = Npclist(NpcIndex)
    ...
    If UserIndex <> 0 And Npclist(NpcIndex).NPCtype = eNPCType.ReydelCastillo Then
   ...
   If UserIndex > 0 Then ' Lo mato un usuario?
        ...
        If Npclist(NpcIndex).flags.Sagrado = 1 Then Call UserPuedeAprender(UserIndex)
        ...
        If MiNPC.flags.ExpCount > 0 Then
            ...
            Else
                If UserList(UserIndex).Pet.Invocada = True Then
                    Call DarExpMascota(UserIndex, (MiNPC.flags.ExpCount / 2))
                    UserList(UserIndex).Stats.Exp = UserList(UserIndex).Stats.Exp + (MiNPC.flags.ExpCount / 2) * ExpX
                    If UserList(UserIndex).Stats.Exp > MAXEXP Then _
                        UserList(UserIndex).Stats.Exp = MAXEXP
                    Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRE51," & MiNPC.flags.ExpCount / 2 * ExpX)
                Else
                UserList(UserIndex).Stats.Exp = UserList(UserIndex).Stats.Exp + MiNPC.flags.ExpCount * ExpX
                If UserList(UserIndex).Stats.Exp > MAXEXP Then _
                    UserList(UserIndex).Stats.Exp = MAXEXP
                Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB4," & MiNPC.flags.ExpCount * ExpX)
            End If
        End If
...
On Error GoTo errhandler
    ...
    Dim MaestroI As Integer
    MaestroI = Npclist(NpcIndex).MaestroUser
    If MaestroI <> 0 Then
        If UserList(MaestroI).Pet.Index = NpcIndex Then
            UserList(MaestroI).Pet.Index = 0
            UserList(MaestroI).Pet.Invocada = False
        End If
    End If
...
Dim Crypted As String
    ...
    Else
        Crypted = "CC" & Npclist(NpcIndex).Char.Body & "," & Npclist(NpcIndex).Char.Head & "," & Npclist(NpcIndex).Char.Heading & "," & Npclist(NpcIndex).Char.CharIndex & "," & X & "," & Y
        ...
        Call SendData(sndRoute, sndIndex, sndMap, "ç" & Encrypt_PRO(Crypted, CRYPTO_PASS))
    End If
...
If NpcIndex > 0 Then
    ...
    Crypted = "CP" & Npclist(NpcIndex).Char.CharIndex & "," & Body & "," & Head & "," & Heading
    If sndRoute = SendTarget.ToMap Then
        Call SendToNpcArea(NpcIndex, "ç" & Encrypt_PRO(Crypted, CRYPTO_PASS)) '"CP" & Npclist(NpcIndex).Char.CharIndex & "," & Body & "," & Head & "," & Heading)
    Else
        Call SendData(sndRoute, sndIndex, sndMap, "ç" & Encrypt_PRO(Crypted, CRYPTO_PASS)) '"CP" & Npclist(NpcIndex).Char.CharIndex & "," & Body & "," & Head & "," & Heading)
    End If
End If
...
'If sndRoute = SendTarget.ToMap Then
   ...
   Call SendData(SendTarget.ToNPCArea, NpcIndex, Npclist(NpcIndex).Pos.Map, "BP" & Npclist(NpcIndex).Char.CharIndex)
'Else
...
#Else
            Crypted = "*" & Npclist(NpcIndex).Char.CharIndex & "," & nPos.X & "," & nPos.Y
            
            Call SendToNpcArea(NpcIndex, "ç" & Encrypt_PRO(Crypted, CRYPTO_PASS)) '"*" & Npclist(NpcIndex).Char.CharIndex & "," & nPos.X & "," & nPos.Y)
#End If
...
If MiNPC.GiveGLD > 0 Then
    Dim MiObj As Obj
    Cantidad = MiNPC.GiveGLD * OroX

    If Cantidad > 100000 Then Cantidad = 100000
        Do While (Cantidad > 0)
...
Npclist(NpcIndex).VeInvis = val(Leer.GetValue("NPC" & NpcNumber, "VeInvis"))
...
Npclist(NpcIndex).Duelos = val(Leer.GetValue("NPC" & NpcNumber, "Duelos"))
...
Npclist(NpcIndex).flags.Sagrado = val(Leer.GetValue("NPC" & NpcNumber, "Sagrado"))
