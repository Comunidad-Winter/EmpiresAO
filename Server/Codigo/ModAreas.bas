...
#End If
                            ...
                            If UserList(TempInt).flags.Invisible Or UserList(TempInt).flags.Oculto Then
                                 Crypted = "NOVER" & UserList(TempInt).Char.CharIndex & ",1" & ENDC
                                 Call EnviarDatosASlot(UserIndex, "ç" & Encrypt_PRO(Crypted, CRYPTO_PASS)) '"NOVER" & UserList(TempInt).Char.CharIndex & ",1" & ENDC)
                            End If
                            
                            If UserList(UserIndex).flags.Invisible Or UserList(UserIndex).flags.Oculto Then
                                 Crypted = "NOVER" & UserList(UserIndex).Char.CharIndex & ",1" & ENDC
                                 Call EnviarDatosASlot(TempInt, "ç" & Encrypt_PRO(Crypted, CRYPTO_PASS)) '"NOVER" & UserList(UserIndex).Char.CharIndex & ",1" & ENDC)
                            End If
#If SeguridadAlkon Then
                        End If
#End If
                ...
                If MapData(Map, X, Y).OBJInfo.ObjIndex Then
                    ...
                    If Not EsObjetoFijo(ObjData(TempInt).OBJType) Then
                        Crypted = "HO" & ObjData(TempInt).GrhIndex & "," & X & "," & Y
                        Call SendData(SendTarget.ToIndex, UserIndex, 0, "ç" & Encrypt_PRO(Crypted, CRYPTO_PASS)) '"HO" & ObjData(TempInt).GrhIndex & "," & X & "," & Y)
