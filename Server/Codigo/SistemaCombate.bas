...
Select Case UCase$(Clase)
    ...
    Case Else
        ModificadorEvasion = 0.4 'el mago no debe evadir ya bastante maquina de matar es
End Select
End Function
...
Select Case UCase$(Clase)
    ...
    Case "CAZADOR"
        ModificadorPoderAtaqueProyectiles = 1.1
    Case "PALADIN"
...
Select Case UCase$(Clase)
    ...
    Case "ASESINO"
        ModicadorDañoClaseProyectiles = 0.8
    Case "LADRON"
        ModicadorDañoClaseProyectiles = 0.95
    Case "PIRATA"
...
If Npclist(NpcIndex).MaestroUser > 0 Then
    If UserList(Npclist(NpcIndex).MaestroUser).Pet.Index = NpcIndex Then
        UserList(Npclist(NpcIndex).MaestroUser).Pet.MinHP = Npclist(NpcIndex).Stats.MinHP
        If UserList(Npclist(NpcIndex).MaestroUser).Pet.MinHP < 0 Then
            UserList(Npclist(NpcIndex).MaestroUser).Pet.MinHP = 0
            Call SendData(SendTarget.ToIndex, Npclist(NpcIndex).MaestroUser, 0, "PRE47")
        End If
    End If
End If
'/PET
...
If Npclist(Victima).MaestroUser > 0 Then
    If UserList(Npclist(Victima).MaestroUser).Pet.Index = Victima Then
        UserList(Npclist(Victima).MaestroUser).Pet.MinHP = Npclist(Victima).Stats.MinHP
        If UserList(Npclist(Victima).MaestroUser).Pet.MinHP < 0 Then
            UserList(Npclist(Victima).MaestroUser).Pet.MinHP = 0
            Call SendData(SendTarget.ToIndex, Npclist(Victima).MaestroUser, 0, "PRE47")
        End If
    End If
End If

If Npclist(Atacante).MaestroUser > 0 Then
    If UserList(Npclist(Atacante).MaestroUser).Pet.Index = Atacante Then
        MaestroIndexI = Npclist(Atacante).MaestroUser
        Call SendData(SendTarget.ToIndex, MaestroIndexI, 0, "||" & vbRed & "°" & daño & "°" & str(Npclist(Victima).Char.CharIndex))
    End If
End If
'/PET
...
If IntervaloPermiteAtacar(UserIndex) Then
    ...
    Else
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRT1")
        Exit Sub
    End If
    ...
    Dim VictimaIndexada As Integer
    ...
    'Look for user
    If Index > 0 Then
        VictimaIndexada = MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).UserIndex
        ...
        Call SendUserStatsBox(VictimaIndexada)
        Exit Sub
    End If
...
Sub UsuarioAtacadoPorUsuario(ByVal AttackerIndex As Integer, ByVal VictimIndex As Integer)
    ...
    If UserList(AttackerIndex).flags.Invisible = 1 Then
        If UserList(AttackerIndex).Counters.Invisibilidad < IntervaloInvisible Then
            UserList(AttackerIndex).Counters.Invisibilidad = IntervaloInvisible
            Call SendData(SendTarget.ToIndex, AttackerIndex, 0, "PRB54")
        End If
    End If
...
For iCount = 1 To MAXMASCOTAS
    If UserList(Maestro).MascotasIndex(iCount) > 0 Then
            If UserList(Maestro).Pet.Index <> UserList(Maestro).MascotasIndex(iCount) Then
...
If UserList(AttackerIndex).flags.Seguro Then
        If Not Criminal(VictimIndex) Then
                Call SendData(SendTarget.ToIndex, AttackerIndex, 0, "PRB74")
                Exit Function
        End If
End If
...
If ExpaDar > 0 Then
    ...
    Else
        If UserList(UserIndex).Pet.Invocada = True Then
            Call DarExpMascota(UserIndex, (ExpaDar / 2))
            UserList(UserIndex).Stats.Exp = UserList(UserIndex).Stats.Exp + (ExpaDar / 2) * ExpX
            If UserList(UserIndex).Stats.Exp > MAXEXP Then _
                UserList(UserIndex).Stats.Exp = MAXEXP
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRE51," & ExpaDar / 2 * ExpX)
        Else
            UserList(UserIndex).Stats.Exp = UserList(UserIndex).Stats.Exp + ExpaDar * ExpX
            If UserList(UserIndex).Stats.Exp > MAXEXP Then _
                UserList(UserIndex).Stats.Exp = MAXEXP
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRE51," & ExpaDar * ExpX)
        End If
    End If
