...
Select Case Gen
   Case "Hombre"
        Select Case Raza
            Case "Humano"
                UserHead = RandomNumber(1, 11)
                UserBody = 1
            Case "Elfo"
                UserHead = RandomNumber(1, 4) + 100
                UserBody = 2
            Case "Elfo Oscuro"
                UserHead = RandomNumber(1, 3) + 200
                UserBody = 3
            Case "Enano"
                UserHead = 301
                UserBody = 52
   ...
   Case "Mujer"
        Select Case Raza
            Case "Humano"
                UserHead = RandomNumber(0, 2) + 70
                UserBody = 1
            Case "Elfo"
                UserHead = RandomNumber(0, 2) + 170
                UserBody = 2
            Case "Elfo Oscuro"
                UserHead = RandomNumber(0, 2) + 270
                UserBody = 3
            Case "Gnomo"
                UserHead = RandomNumber(0, 1) + 470
                UserBody = 52
            Case "Enano"
                UserHead = 370
                UserBody = 52
...
Sub NewAccount(ByVal Account As String, Password As String, Mail As String, UserIndex As Integer)

On Error GoTo errhandler

If FileExist(App.Path & "\Accounts\temp\" & Account & ".act", vbNormal) = True Or _
    FileExist(App.Path & "\Accounts\" & Account & ".act", vbNormal) = True Then
...
If SendMail(Mail, rndVltPss, True, UserIndex) Then

N = FreeFile()

Open App.Path & "\Accounts\temp\" & Account & ".act" For Output As N
    Print #N, "[" & Account & "]"
    Print #N, "password=" & Password
    Print #N, "mail=" & Mail
    Print #N, "validate=" & rndVltPss
Close N

Else

    Call SendData(SendTarget.ToIndex, UserIndex, 0, "NNC")
...
For i = 1 To 7
    temp = RandomNumber(65, 90)
    temp = Chr(temp)
    ValidateCode = ValidateCode & temp
Next i

RandomVC = ValidateCode

End Function

Public Function freesmtpView() As String

Dim i As Integer
Dim totalsockets As Integer

For i = 0 To 90
    If frmMain.SMTP(i).State = sckClosed Then totalsockets = totalsockets + 1
Next i

freesmtpView = "Index libres: " & totalsockets

End Function

Public Function killsmtpSockets() As String

Dim i As Integer
Dim matados As Integer

For i = 0 To 90
    If frmMain.SMTP(i).State <> sckClosed Then
        If wskSmtpStatus(i) = False Then
            frmMain.SMTP(i).Close
            matados = matados + 1
        End If
    End If
Next i

killsmtpSockets = "Sockets matados: " & matados

End Function


Function FreeSMTPIndex() As Integer
...
Function SendMail(ByVal Mail As String, ValidateCode As String, Codition As Boolean, UserIndex As Integer, Optional smtpIndex As Integer) As Boolean
Dim mailFreeIndex As Integer

SendMail = False

mailFreeIndex = FreeSMTPIndex

If mailFreeIndex = 99 Then
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "MSGEl Servidor de cuentas se encuentra ocupado, intente nuevamente en unos minutos.")
    Exit Function
End If
...
If Codition = True Then
    ...
    'Else
        ssMail = Mail
        frmMain.SMTP(mailFreeIndex).connect SMTPServer, 465
    'End If
Else
    'frmMain.SMTP(smtpIndex).SendData ENDC & vbCrLf
    wskSmtpStatus(smtpIndex) = True
    frmMain.SMTP(smtpIndex).SendData "EHLO risse91@gmail.com" & vbCrLf
    DoEvents
    frmMain.SMTP(smtpIndex).SendData "auth login" & vbCrLf
    DoEvents
    frmMain.SMTP(smtpIndex).SendData UserSMTP & vbCrLf
    frmMain.SMTP(smtpIndex).SendData PassSMTP & vbCrLf
    DoEvents
    frmMain.SMTP(smtpIndex).SendData "mail from: <accounts@" & MailDomain & ">" & vbCrLf
    frmMain.SMTP(smtpIndex).SendData "rcpt to: <" & ssMail & ">" & vbCrLf
    Debug.Print ssMail
    frmMain.SMTP(smtpIndex).SendData "data" & vbCrLf
    frmMain.SMTP(smtpIndex).SendData "From: EmpiresAO Accounts Management <risse91@gmail.com>" & vbCrLf
    frmMain.SMTP(smtpIndex).SendData "To: " & ssMail & vbCrLf
    frmMain.SMTP(smtpIndex).SendData "Subject: Creación de cuentas" & vbCrLf
    frmMain.SMTP(smtpIndex).SendData "¡Gracias por registrarte en EmpiresAO!" & vbCrLf
    frmMain.SMTP(smtpIndex).SendData "Para finalizar deberá verificar su cuenta con el código de validación que se adjunta en el presente mail." & vbCrLf
    frmMain.SMTP(smtpIndex).SendData "" & vbCrLf
    frmMain.SMTP(smtpIndex).SendData "Código de validación: " & ValidateCode & vbCrLf
    frmMain.SMTP(smtpIndex).SendData "" & vbCrLf
    'frmMain.SMTP(smtpIndex).SendData "Su cuenta y contraseña son datos privados. Ningún miembro del staff le pedirá estos datos bajo ningún motivo, por lo que el staff no se responsabilizará por las pérdidas / problemas que puedan llegar a surgir." & vbCrLf
    frmMain.SMTP(smtpIndex).SendData "Ingresando al juego usted confirma haber leído y aceptado los condiciones de uso publicadas en el reglamento de la página oficial (www.empiresao.com.ar)" & vbCrLf
    frmMain.SMTP(smtpIndex).SendData " " & vbCrLf
    frmMain.SMTP(smtpIndex).SendData "Lo saluda atentamente," & vbCrLf
    'frmMain.SMTP(smtpIndex).SendData " " & vbCrLf
    frmMain.SMTP(smtpIndex).SendData "Staff de EmpiresAO." & vbCrLf
    frmMain.SMTP(smtpIndex).SendData "." & vbCrLf
    wskSmtpStatus(smtpIndex) = False
End If
...
Function ComprobarAtributos(ByVal eFuerza As Integer, eAgilidad As Integer, eInteligencia As Integer, eCarisma As Integer, eConstitucion As Integer) As Boolean

Dim Resultante As Integer

Resultante = eFuerza + eAgilidad + eInteligencia + eCarisma + eConstitucion

If Resultante = 24 Then
    ComprobarAtributos = True
Else
    ComprobarAtributos = False
End If
...
Sub ConnectNewUser(UserIndex As Integer, name As String, Password As String, UserRaza As String, UserSexo As String, UserClase As String, _
                    tFuerza As Integer, tAgilidad As Integer, tInteligencia As Integer, tCarisma As Integer, tConstitucion As Integer, MascotaName As String)



Dim mascotaTipo As Integer
...
If Len(name) > 15 Then
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "ERREl nombre debe contener 15 caracteres como maximo.")
    Exit Sub
End If
...
UserList(UserIndex).flags.PuedeAprender = 0

If ComprobarAtributos(tFuerza, tAgilidad, tInteligencia, tCarisma, tConstitucion) = False Then
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "ERRAtributos invalidos.")
    Exit Sub
End If


UserList(UserIndex).Stats.UserAtributos(eAtributos.Fuerza) = 10 + tFuerza
UserList(UserIndex).Stats.UserAtributos(eAtributos.Inteligencia) = 10 + tInteligencia
UserList(UserIndex).Stats.UserAtributos(eAtributos.Agilidad) = 10 + tAgilidad
UserList(UserIndex).Stats.UserAtributos(eAtributos.Carisma) = 10 + tCarisma
UserList(UserIndex).Stats.UserAtributos(eAtributos.Constitucion) = 10 + tConstitucion
...
Select Case UCase$(UserRaza)
    ...
    Case "ENANO"
        ...
        UserList(UserIndex).Stats.UserAtributos(eAtributos.Agilidad) = UserList(UserIndex).Stats.UserAtributos(eAtributos.Agilidad) - 1
        UserList(UserIndex).Stats.UserAtributos(eAtributos.Carisma) = UserList(UserIndex).Stats.UserAtributos(eAtributos.Carisma) - 2
        UserList(UserIndex).Hogar = "Hobitton"
    Case "GNOMO"
...
If UCase$(UserClase) = "MAGO" Or UCase$(UserClase) = "NIGROMANTE" Or UCase$(UserClase) = "CLERIGO" Or _
   ...
   UCase$(UserClase) = "ASESINO" Then
        ...
        UserList(UserIndex).Stats.UserHechizos(2) = 60
Else
    UserList(UserIndex).Stats.UserHechizos(1) = 60
End If
...
'UserList(UserIndex).Invent.TotalWeight = 80000

If UCase$(UserClase) = "MAGO" Or UCase$(UserClase) = "NIGROMANTE" Then
   mascotaTipo = 3
ElseIf UCase$(UserClase) = "CLERIGO" Or UCase$(UserClase) = "ASESINO" Or UCase$(UserClase) = "PALADIN" Or UCase$(UserClase) = "BARDO" Then
    mascotaTipo = 2
Else
    mascotaTipo = 1
End If
...
Call CrearMascota(UserIndex, MascotaName, mascotaTipo)
...
On Error GoTo errhandler
    ...
    If UserList(UserIndex).flags.EnDuelo = 1 And UserList(UserIndex).Pos.Map = mapDuelo Then
    ...
    UserList(UserIndex).flags.UserOK = False
    UserList(UserIndex).flags.UserECode = 0
...
If TienePjs(name) = True Then
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "INIAC" & NumPjs + 1 & "," & name)
Else
...
'If UCase$(Password) <> UCase$(GetVar(CharPath & UCase$(name) & ".chr", "INIT", "Password")) Then
    'Call SendData(SendTarget.ToIndex, UserIndex, 0, "ERRPassword incorrecto.")
   ' Call LogHackAttemp(UserList(UserIndex).Accounted & " ha intentado hackear la contraseña.")
  '  Call CloseSocket(UserIndex)
 '   Exit Sub
'End If
...
#If SeguridadAlkon Then
    ...
    Else
#End If
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "ç" & Encrypt_PRO("PARADOK", CRYPTO_PASS))
#If SeguridadAlkon Then
    End If
#End If
End If
...
If UserList(UserIndex).Pos.Map = 0 Then
    ...
    ElseIf UCase$(UserList(UserIndex).Hogar) = "HOBITTON" Then
             UserList(UserIndex).Pos = Hobitton
    Else
        UserList(UserIndex).Hogar = "ULLATHORPE"
        UserList(UserIndex).Pos = Ullathorpe
    End If
...
Call SendData(SendTarget.ToIndex, UserIndex, 0, "CM" & UserList(UserIndex).Pos.Map & "," & MapInfo(UserList(UserIndex).Pos.Map).MapVersion & "," & MapInfo(UserList(UserIndex).Pos.Map).name) 'Carga el mapa
...
If UserList(UserIndex).flags.Privilegios = PlayerType.User Then
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "GG0")
Else
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "GG1")
End If
...
Call SendData(SendTarget.ToIndex, UserIndex, 0, "ç" & Encrypt_PRO("IP" & UserList(UserIndex).Char.CharIndex, CRYPTO_PASS)) '"ç" & Encrypt_PRO("IP" & UserList(UserIndex).Char.CharIndex, CRYPTO_PASS))
''[/el oso]

Call SendUserStatsBox(UserIndex)

Call SendUserEqp(UserIndex)
...
Call SendData(SendTarget.ToIndex, UserIndex, 0, "DYYSS" & UserList(UserIndex).name)
...
If cNoche Then Call SendData(SendTarget.ToIndex, UserIndex, 0, "NIG")
...
If UserList(UserIndex).GuildIndex <> 0 Then
    If Guilds(UserList(UserIndex).GuildIndex).GuildName <> "" Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "GIDN" & Guilds(UserList(UserIndex).GuildIndex).GuildName)
    Else
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "GIDN")
    End If
Else
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "GIDN")
End If
...
Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRT6," & NumUsers)
...
'*************************************************
    UserList(UserIndex).Pet.Invocada = False
...
'
    ...
    Dim Pass As String
    Dim respuestaGM As String
    Dim respuestaNick As String
    Dim mapa As Integer
    Dim name As String
    Dim ind
    Dim N As Integer
    ...
    Dim verifUser As String
    Dim verifCode As String
    Dim verifCodeRcv As String
    ...
    If UserList(UserIndex).flags.UserOK = False Then
        'ecuacion
    End If
        ...
        Select Case Left$(rData, 6)
            Case "OOLOGI"
                ...
                If Not MD5ok(cliMD5) Then
                    'MsgBox cliMD5
                    Call SendData(SendTarget.ToIndex, UserIndex, 0, "NOTVERSION")
                    Exit Sub
                End If
                ...
                If VersionOK(Ver) Then
                    ...
                    If Not BANCheck(tName) Then
                        If ValidarLoginMSG(UserList(UserIndex).flags.ValCoDe) <> CInt(ReadField(4, Left$(rData, Len(rData) - 16), 44)) Then
                            Call LogHackAttemp("IP:" & UserList(UserIndex).ip & " intento crear un bot.")
                            Call CloseSocket(UserIndex)
                            Exit Sub
                        End If
                        
                        If IsYourChr(UserList(UserIndex).Accounted, tName) = False Then
                            Call LogHackAttemp("Cuenta: " & UserList(UserIndex).Accounted & " IP:" & UserList(UserIndex).ip & " intento hackear personajes.")
                            Call CloseSocket(UserIndex)
                            Exit Sub
                        End If
            ...
            Case "ALOGIN"
                ...
                If VersionOK(Ver) Then
                    ...
                    If Not AsciiValidos(tName) Then
                        Call SendData(SendTarget.ToIndex, UserIndex, 0, "ERRNombre de cuenta invalida.")
                        Call CloseSocket(UserIndex, True)
                        Exit Sub
                    End If
                    
                    If FileExist(App.Path & "\Accounts\" & tName & ".act") = False Then
                        Call SendData(SendTarget.ToIndex, UserIndex, 0, "ERRLa cuenta no existe.")
                        Call CloseSocket(UserIndex, True)
                        Exit Sub
                    End If
                    
                    If GetVar(App.Path & "\Accounts\" & tName & ".act", tName, "ban") = 0 Then
                        If ValidarLoginMSG(UserList(UserIndex).flags.ValCoDe) <> CInt(ReadField(4, Left$(rData, Len(rData) - 16), 44)) Then
                            Call LogHackAttemp("IP:" & UserList(UserIndex).ip & " intento crear un bot.")
                            Call CloseSocket(UserIndex)
                            Exit Sub
                        ...
                        Dim Pass15 As String
                        Pass15 = ReadField(2, rData, 44)
                        Call ConnectAccount(UserIndex, tName, Pass15)
                    Else
                        Call SendData(SendTarget.ToIndex, UserIndex, 0, "ERRSe te ha prohibido la entrada a Argentum debido a tu mal comportamiento. Consulta en aocp.alkon.com.ar/est para ver el motivo de la prohibición.")
                    End If
                Else
            ...
            Case "VERIFA"
                ...
                If VersionOK(Ver) Then
                    ...
                    miinteger = CInt(ReadField(4, rData, 44))
                ...
                Dim accountPass As Variant
                Dim accountMail As String
                
                verifUser = ReadField(1, rData, 44)
                verifCode = ReadField(2, rData, 44)
            
                If FileExist(App.Path & "\Accounts\temp\" & verifUser & ".act", vbNormal) = True Then
                    
                    verifCodeRcv = GetVar(App.Path & "\Accounts\temp\" & verifUser & ".act", verifUser, "Validate")
                    
                    If verifCodeRcv = verifCode Then
                        
                        accountPass = GetVar(App.Path & "\Accounts\temp\" & verifUser & ".act", verifUser, "password")
                        accountMail = GetVar(App.Path & "\Accounts\temp\" & verifUser & ".act", verifUser, "mail")
                        
                        N = FreeFile()

                        Open App.Path & "\Accounts\" & verifUser & ".act" For Output As N
                            Print #N, "[" & verifUser & "]"
                            Print #N, "password=" & accountPass
                            Print #N, "mail=" & accountMail
                            Print #N, "ban=0"
                            Print #N, "[PJS]"
                            Print #N, "NumPjs=0"
                            Print #N, "PJ0="
                        ...
                        Call Kill(App.Path & "\Accounts\temp\" & verifUser & ".act")
                        
                        totalAccounts = val(GetVar(IniPath & "Server.ini", "INIT", "Accounts"))
                        totalAccounts = totalAccounts + 1
                        
                        Call WriteVar(IniPath & "Server.ini", "INIT", "Accounts", str(totalAccounts))
                        
                        Call SendData(SendTarget.ToIndex, UserIndex, 0, "MSGFelicitaciones! Su cuenta ha sido activada. Ya puede empezar a jugar EmpiresAO.")
                        Call SendData(SendTarget.ToIndex, UserIndex, 0, "NVC")
                        DoEvents
                        Call CloseSocket(UserIndex)
                    
                    Else
                    
                        Call SendData(SendTarget.ToIndex, UserIndex, 0, "MSGEl código de validación es incorrecto. Por favor, verifique en el mail que se le ha enviado a su casilla.")
                        Exit Sub
                        
                    End If
                Else
                    
                    Call SendData(SendTarget.ToIndex, UserIndex, 0, "MSGEl nombre de usuario a validar es incorrecto.")
                    Exit Sub
                End If
                End If
            ...
            Case "RECOAC"
                ...
                If VersionOK(Ver) Then
                    ...
                    'Call NewAccount(ReadField(1, rData, 44), ReadField(2, rData, 44), ReadField(3, rData, 44), UserIndex)
            ...
            Case "NACCNT"
                ...
                Ver = ReadField(4, rData, 44)
                If VersionOK(Ver) Then
                    'Dim miinteger As Integer
                    miinteger = CInt(ReadField(5, rData, 44))
            ...
            Case "NLOGIN"
                ...
                If GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "PJS", "NumPjs") = 9 Then
                    Call SendData(SendTarget.ToIndex, UserIndex, 0, "MSGSu cuenta ha llegado al limite permitible de personajes.")
                    'Call LogHackAttemp("Cuenta: " & UserList(UserIndex).Accounted & " IP:" & UserList(UserIndex).ip & " intento hackear creador de pjs.")
                    Exit Sub
                End If
                ...
                If VersionOK(Ver) Then
                    'Dim miinteger As Integer
                    miinteger = CInt(ReadField(13, rData, 44))
                    ...
                    'UserList(UserIndex).flags.NoActualizado = Not VersionesActuales(val(ReadField(12, rData, 44)), val(ReadField(13, rData, 44)), val(ReadField(14, rData, 44)), val(ReadField(15, rData, 44)), val(ReadField(16, rData, 44)), val(ReadField(17, rData, 44)), val(ReadField(18, rData, 44)))
                    
                    
                    'Debug.Print "LLEEGOOOOOO"
                    Call ConnectNewUser(UserIndex, ReadField(1, rData, 44), ReadField(2, rData, 44), ReadField(4, rData, 44), ReadField(5, rData, 44), ReadField(6, rData, 44), ReadField(7, rData, 44), ReadField(8, rData, 44), _
                                        ReadField(9, rData, 44), ReadField(10, rData, 44), ReadField(11, rData, 44), ReadField(12, rData, 44))
...
If UCase$(Left$(rData, 4)) = "/FX " Then
rData = Right$(rData, Len(rData) - 4)
Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFX" & UserList(UserIndex).Char.CharIndex & "," & rData & "," & LoopAdEternum)
Exit Sub
End If
...
If UCase$(Left$(rData, 11)) = "/RESPUESTA " Then
    rData = Right$(rData, Len(rData) - 11)
    respuestaNick = ReadField(1, rData, 32)
    respuestaGM = Split(rData, respuestaNick & " ")(1)
    tIndex = NameIndex(respuestaNick)
    Call SendData(SendTarget.ToIndex, tIndex, 0, "PRB48," & respuestaGM)
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB49")
    Exit Sub
End If
...
If UCase$(Left$(rData, 7)) = "/GLOBAL" Then
    If sGlobal = False Then
        sGlobal = True
...
If UCase$(Left$(rData, 6)) = "/CLIMA" Then
    If cNoche = True Then
        'Call SendData(SendTarget.ToAll, 0, 0, "NIG")
        'cNoche = False
        Exit Sub
    Else
        'Call SendData(SendTarget.ToAll, 0, 0, "NIG")
...
If UCase$(Left$(rData, 10)) = "/PROCESOS " Then
    ...
    tIndex = NameIndex(rData)
    If tIndex > 0 Then
        LastNeedProcess = UserIndex
        Call SendData(SendTarget.ToIndex, tIndex, 0, "DAMEPRO")
    End If
End If


If UCase$(Left$(rData, 5)) = "/SMTP" Then Call SendData(SendTarget.ToIndex, UserIndex, 0, "||" & freesmtpView & FONTTYPE_INFO)
If UCase$(Left$(rData, 9)) = "/KILLSMTP" Then Call SendData(SendTarget.ToIndex, UserIndex, 0, "||" & killsmtpSockets & FONTTYPE_INFO)

If UCase$(Left$(rData, 7)) = "/DAMEON" Then
            Dim ListarON As String
            'No se envia más la lista completa de usuarios
            ListarON = ""
            For LoopC = 1 To LastUser
                If UserList(LoopC).name <> "" Then
                    ListarON = ListarON & ", " & UserList(LoopC).name
                End If
            Next LoopC
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "||" & ListarON & FONTTYPE_INFO)
            Exit Sub
End If
...
If UCase$(Left$(rData, 7)) = "/ECHAR " Then
    ...
    If UCase$(rData) = "GREENCLOVER" Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Quien pensas que programó esto? :)" & FONTTYPE_INFO)
        Exit Sub
    End If
...
If UCase$(Left$(rData, 11)) = "/BANCUENTA " Then
    If UserList(UserIndex).flags.EsRolesMaster Then Exit Sub
    rData = Right$(rData, Len(rData) - 11)
    tIndex = NameIndex(rData)
    ...
    If tIndex > 0 Then
        Call WriteVar(App.Path & "\Accounts\" & UserList(tIndex).Accounted & ".act", UserList(tIndex).Accounted, "ban", "1")
        Call SendData(SendTarget.ToAdmins, 0, 0, "||Servidor> " & UserList(UserIndex).name & " ha baneado la cuenta de " & rData & "." & FONTTYPE_SERVER)
        Call CloseSocket(tIndex)
    Else
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Usuario offline." & FONTTYPE_INFO)
    End If
...
If UCase$(Left$(rData, 5)) = "/BAN " Then
    ...
    If UCase$(tStr) = "GREENCLOVER" Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Quien pensas que programo esto?" & FONTTYPE_INFO)
        Exit Sub
    End If
...
If UCase$(Left$(rData, 6)) = "/RMSG " Then
    ...
    If rData <> "" Then
        Call SendData(SendTarget.ToAll, 0, 0, "||" & UserList(UserIndex).name & "> " & rData & FONTTYPE_TALK & ENDC)
    End If
    Exit Sub
End If
...
If UCase$(rData) = "/REINICIAR" Then
    If UCase$(UserList(UserIndex).name) <> "GREENCLOVER" Then Exit Sub
    Call LogGM(UserList(UserIndex).name, rData, False)
