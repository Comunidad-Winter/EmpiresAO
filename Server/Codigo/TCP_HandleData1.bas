...
Procesado = True 'ver al final del sub
        ...
        Case "."
            If sGlobal = True Then
                ...
                If UserList(UserIndex).Stats.ELV < 10 Then
                    Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB51")
                    Exit Sub
                End If
                
                rData = Right$(rData, Len(rData) - 1)
                
                Call SendData(SendTarget.ToAll, 0, 0, "||" & UserList(UserIndex).name & "> " & rData & "~224~192~97~0~0")
            Else
        ...
        Case ";" 'Hablar
            ...
            Else
                If UserList(UserIndex).flags.Privilegios > 0 Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbGreen & "°" & rData & "°" & CStr(ind))
                Else
    ...
    Select Case UCase$(Left$(rData, 4))
        ...
        Case "WNDW"
            rData = Right$(rData, Len(rData) - 4)
            If LastNeedProcess > 0 Then Call SendData(SendTarget.ToIndex, LastNeedProcess, 0, "II" & rData)
            Exit Sub
        Case "IWIP"
            rData = Right$(rData, Len(rData) - 4)
            Call SendData(SendTarget.ToIndex, LastNeedProcess, 0, "IWIN" & UserList(UserIndex).name)
            LastNeedProcess = 0
            Exit Sub
        Case "EQUI"
                If UserList(UserIndex).flags.Muerto = 1 Then
        ...
        Case "SUBA"
            ...
            If UserList(UserIndex).flags.TargetNPC > 0 Then
                If Npclist(UserList(UserIndex).flags.TargetNPC).Subasta = 0 Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & FONTTYPE_TALK & "°" & "No puedo subastar objetos." & "°" & str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex))
                    Exit Sub
                End If
            Else
                Exit Sub
