...
Procesado = True 'ver al final del sub
    ...
    Select Case UCase$(rData)
        ...
        Case "/MASCOTA"
            'nombre, elu, elv, exp, defensa, minhp, maxhp, minhit, maxhit, tipo
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "MSTI" & UserList(UserIndex).Pet.Nombre & "," & UserList(UserIndex).Pet.ELU & "," & _
            UserList(UserIndex).Pet.ELV & "," & UserList(UserIndex).Pet.Exp & "," & UserList(UserIndex).Pet.Defensa & "," & _
            UserList(UserIndex).Pet.MinHP & "," & UserList(UserIndex).Pet.MaxHP & "," & UserList(UserIndex).Pet.MinHIT & "," & _
            UserList(UserIndex).Pet.MaxHIT)
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "MSTF" & UserList(UserIndex).Pet.Tipo)
            Exit Sub
           ...
           UserList(UserIndex).flags.Meditando = Not UserList(UserIndex).flags.Meditando
            ...
            If UserList(UserIndex).flags.Meditando Then
                ...
                ElseIf UserList(UserIndex).Stats.ELV < 55 Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFX" & UserList(UserIndex).Char.CharIndex & "," & FXIDs.FXMEDITARmayor & "," & LoopAdEternum)
                    UserList(UserIndex).Char.FX = FXIDs.FXMEDITARmayor
                Else
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFX" & UserList(UserIndex).Char.CharIndex & "," & FXIDs.FXMEDITARmayor & "," & LoopAdEternum)
                    UserList(UserIndex).Char.FX = FXIDs.FXMEDITARmayor
                End If
        ...
        Case "/SUBASTAR"
            ...
            If UserList(UserIndex).flags.TargetNPC > 0 Then
                If Npclist(UserList(UserIndex).flags.TargetNPC).Subasta = 0 Then
                    If Len(Npclist(UserList(UserIndex).flags.TargetNPC).Desc) > 0 Then Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbWhite & "°" & "Te has equivocado de persona, el subastador se encuentra en Ullathorpe." & "°" & CStr(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex))
                    Exit Sub
                End If
                If Distancia(Npclist(UserList(UserIndex).flags.TargetNPC).Pos, UserList(UserIndex).Pos) > 3 Then
        ...
        Case "/DUELO"
            ...
            End If
            If UserList(UserIndex).flags.TargetNPC > 0 Then
                If Npclist(UserList(UserIndex).flags.TargetNPC).Duelos = 0 Then
                    If Len(Npclist(UserList(UserIndex).flags.TargetNPC).Desc) > 0 Then Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbWhite & "°" & "No soy organizador de duelos." & "°" & CStr(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex))
                    Exit Sub
                End If
                If Distancia(Npclist(UserList(UserIndex).flags.TargetNPC).Pos, UserList(UserIndex).Pos) > 3 Then
                    Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Estas demasiado lejos del organizador de duelos." & FONTTYPE_INFO)
                    Exit Sub
                End If
                If UserList(UserIndex).flags.Navegando = 1 Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbWhite & "°" & "No puedes ingresar navegando a la sala de duelos." & "°" & CStr(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex))
                    Exit Sub
                End If
                If UserList(UserIndex).flags.Invisible = 1 Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbWhite & "°" & "No puedes ingresar invisible a la sala de duelos." & "°" & CStr(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex))
                    Exit Sub
                End If
                If UserList(UserIndex).Stats.ELV < 20 Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbWhite & "°" & "Para ingresar al duelo tenes que ser nivel 20 o mayor." & "°" & CStr(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex))
                    Exit Sub
                End If
                If UserList(UserIndex).Stats.GLD < 10000 Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbWhite & "°" & "No tienes suficiente dinero para pagar el duelo." & "°" & CStr(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex))
                    Exit Sub
                End If
                If UserList(UserIndex).NroMacotas > 0 Then
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbWhite & "°" & "No puedes ingresar con mascotas a la sala de duelos." & "°" & CStr(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex))
                    Exit Sub
                End If
                If MapInfo(mapDuelo).NumUsers <= 1 Then
                    Call IngresarDuelo(UserIndex)
                Else
                    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbWhite & "°" & "La sala de duelos está ocupada, debes esperar a que finalice para entrar." & "°" & CStr(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex))
                    Exit Sub
                End If
            Else
                Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB1")
        ...
        Case "/GM "
            ...
            If rData <> "" Then
                If Not Ayuda.Existe(UserList(UserIndex).name) Then
                    ...
                    Call SendData(SendTarget.ToAdmins, 0, 0, "||" & UserList(UserIndex).name & " ha enviado pedido de GM: """ & rData & """~0~234~117~1~0")
                Else
                ...
                End If
            Else
                Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB47")
            End If
            Exit Sub
    End Select
    ...
    Select Case UCase$(Left$(rData, 8))
        Case "/PASSWD "
            ...
            Else
                If LookNewPasswd(rData) = True Then
                    Call SendData(SendTarget.ToIndex, UserIndex, 0, "||El password ha sido cambiado." & FONTTYPE_INFO)
                    Call WriteVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", UserList(UserIndex).Accounted, "password", rData)
                    Call WriteVar(App.Path & "\Charfile\" & UserList(UserIndex).name & ".chr", "INIT", "password", rData)
                Else
                    Call SendData(SendTarget.ToIndex, UserIndex, 0, "||El password contiene caracteres invalidos." & FONTTYPE_INFO)
                End If
            End If
            Exit Sub
    End Select
    ...
    Select Case UCase$(Left$(rData, 9))
        ...
        Case "/APOSTAR "
            ...
            If OfertaSUB < 16000000 Then
                Call SendData(SendTarget.ToIndex, UserIndex, 0, "||No puedes ofertar tanto dinero." & FONTTYPE_INFO)
                Exit Sub
            End If
