...
Begin VB.Form frmMain 
   ...
   Begin VB.Frame Frame2 
      ...
      Top             =   1800
      Width           =   3855
      Begin MSWinsockLib.Winsock wskC 
         Left            =   3120
         Top             =   360
         _ExtentX        =   741
         _ExtentY        =   741
   ...
   Begin VB.Frame Frame1 
      ...
      Top             =   600
      Width           =   4935
      Begin MSWinsockLib.Winsock acptStat 
         Left            =   4080
         Top             =   480
         _ExtentX        =   741
         _ExtentY        =   741
         _Version        =   393216
      End
      Begin MSWinsockLib.Winsock webStats 
         Left            =   3720
         Top             =   360
         _ExtentX        =   741
         _ExtentY        =   741
         _Version        =   393216
      End
      Begin VB.Timer CastillosPremios 
         Interval        =   60000
         Left            =   2880
         Top             =   120
      End
...
Private Sub CastillosPremios_Timer()

Static Horas As Integer
Static rndMin As Integer
...
If PrimeraVez = False Then
    If Minutos = 60 + 24 Then
        Minutos = 0
        rndMin = RandomNumber(25, 55)
        PrimeraVez = True
        Call DarPremioCastillos
    End If
Else
    If Minutos = 60 + rndMin Then
        Minutos = 0
        rndMin = RandomNumber(25, 55)
        Call DarPremioCastillos
    End If
...
Call SendData(ToAll, 0, 0, "NIG")
End Sub

Private Sub Command4_Click()

If wskC.State <> sckClosed Then wskC.Close

lstServer.AddItem "Conectando al servidor de publicaciones..."
wskC.connect "127.0.0.1", 77
...
If lPermitePotear < IntervaloUserPuedePotear Then
    lPermitePotear = lPermitePotear + 1
End If
...
If Not lPermitePotear < IntervaloUserPuedePotear Then
    lPermitePotear = 0
End If
...
SMTP(Index).Close
wskSmtpStatus(Index) = False

End Sub

Private Sub SMTP_Connect(Index As Integer)
Call SendMail("s", rndVltPss, False, 0, Index)
End Sub
...
SMTP(Index).GetData sData

Debug.Print "I:" & Index & " - " & sData

If InStr(UCase(sData), "503 ERROR") Or InStr(UCase(sData), "502 ERROR") Or _
InStr(UCase(sData), "501 SYNTAX") Or InStr(UCase(sData), "535 ERROR") Or InStr(UCase(sData), "ERROR:") Then
    Debug.Print "ERROR: " & Index & "-----" & sData
    SMTP(Index).Close
End If
...
Private Sub SMTP_Error(Index As Integer, ByVal Number As Integer, Description As String, ByVal Scode As Long, ByVal Source As String, ByVal HelpFile As String, ByVal HelpContext As Long, CancelDisplay As Boolean)
...
If Minutoss >= 4 Then
    ...
    Else
       ...
       ' Call SendData(ToPCArea, GranPoder, UserList(GranPoder).Pos.Map, "CFX")
         'Call SendData(ToPCArea, GranPoder, UserList(GranPoder).Pos.Map, "CFX" & UserList(GranPoder).Char.CharIndex & "," & FXGRANPODER & "," & 0)
    End If
'Else
    'If GranPoder > 0 Then Call SendData(ToPCArea, GranPoder, UserList(GranPoder).Pos.Map, "CFX" & UserList(GranPoder).Char.CharIndex & "," & FXGRANPODER & "," & 0)
    'Call SendData(ToPCArea, GranPoder, UserList(GranPoder).Pos.Map, "CFX")
End If
End Sub
...
If i = 30 Then
    ...
    For E = 0 To 90
        If SMTP(E).State <> sckClosed Then
            If wskSmtpStatus(E) = False Then
                SMTP(E).Close
            End If
        End If
    Next E
...
frmMain.transferSeg.Caption = Round(transferTotal / 1024, 3) & " KB/s"

If transferTotal > transferTotalMax Then
    transferTotalMax = transferTotal
    frmMain.transferMax.Caption = Round(transferTotalMax / 1024, 3) & " KB/s"
End If
...
For i = 1 To LastUser
    If UserList(i).flags.UserLogged Then
            ...
            If MapData(UserList(i).Pos.Map, UserList(i).Pos.X, UserList(i).Pos.Y).trigger = eTrigger.ANTIPIQUETE Then
                    ...
                    Call SendData(SendTarget.ToIndex, i, 0, "PRE18")
...
Private Sub webStats_ConnectionRequest(ByVal requestID As Long)
...
acptStat.accept requestID
acptStat.SendData "<html><style type=""text/css""><!--body,td,th { font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 8px; color: #000000; font-weight: bold; }" & vbCrLf
acptStat.SendData "body { background-color: #F5F3ED; } --> </style></head> <p>Jugadores Online: " & NumUsers & "<br> N&uacute;mero de cuentas: " & totalAccounts & "<br> N&uacute;mero de personajes: " & totalPjs & "<br>Record online: " & recordusuarios & "</p></html>" & vbCrLf
'acptStat.SendData "<html><b>" & NumUsers & "</b></html>" & vbCrLf
DoEvents
DoEvents
'DoEvents
acptStat.Close
...
Private Sub webStats_DataArrival(ByVal bytesTotal As Long)
Dim data As String
webStats.GetData data
'List1.AddItem data
End Sub
...
'''''''''''''Compilar con UsarQueSocket = 3''''''''''''''''''''''''
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Private Sub webStats_Error(ByVal Number As Integer, Description As String, ByVal Scode As Long, ByVal Source As String, ByVal HelpFile As String, ByVal HelpContext As Long, CancelDisplay As Boolean)

End Sub

Private Sub wskC_Close()

lstServer.AddItem "Se ha perdido conexión con el servidor."
...
Dim ValCode2 As Long
Dim wskData As String

wskC.GetData wskData

Select Case UCase$(Left$(wskData, 5))
    Case "ERROR"
    If Split(wskData, "ERROR")(1) = 731 Then
        MsgBox "No se ha podido conectar al Servidor central. Posiblemente se encuentre lleno.", vbCritical
        wskC.Close
    End If
    Exit Sub
    Case "VLTOK"
    lstServer.AddItem "Su EAOmod ha sido publicado satisfactoriamente."
    Exit Sub
End Select

Select Case UCase$(Left$(wskData, 1))
    Case "Q"
    wskData = Split(wskData, "Q")(1)
    ValCode2 = ValidarLoginMSG(wskData)
    wskC.SendData "ADD" & ServerNombre & "," & Puerto & "," & ServerWeb & "," & ValCode2 & vbCrLf
    Exit Sub
End Select
