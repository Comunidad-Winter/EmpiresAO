...
Private Const NPC_CENTINELA_TIERRA As Integer = 164  'Índice del NPC en el .dat
Private Const NPC_CENTINELA_AGUA As Integer = 164     'Ídem anterior, pero en mapas de agua
...
'############################################################
    ...
    For LoopC = 1 To LastUser
        If UserList(LoopC).name <> "" And UserList(LoopC).Counters.Trabajando > 0 Then 'And UserList(LoopC).flags.Privilegios = PlayerType.User Then
...
Public Sub CentinelaCheckClave(ByVal UserIndex As Integer, ByVal clave As Long)
'############################################################
