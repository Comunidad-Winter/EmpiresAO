...
If Hechizos(Spell).SubeHP = 1 Then
    ...
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB21," & Npclist(NpcIndex).name & "," & daño)
...
If hIndex = 48 Or hIndex = 49 Or hIndex = 50 Or hIndex = 51 Or hIndex = 52 Or hIndex = 53 Or hIndex = 54 Or _
hIndex = 55 Or hIndex = 56 Or hIndex = 57 Or hIndex = 58 Or hIndex = 59 Then
    If UserList(UserIndex).flags.PuedeAprender = 0 Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB37")
        Exit Sub
    End If
    If UCase$(Hechizos(hIndex).SoloClase) <> UCase$(UserList(UserIndex).Clase) Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB38")
        Exit Sub
    End If
End If
...
If UserList(UserIndex).Pet.Invocada = False And UserList(UserIndex).NroMacotas >= MAXMASCOTAS Then Exit Sub

H = UserList(UserIndex).Stats.UserHechizos(UserList(UserIndex).flags.Hechizo)

'No permitimos se invoquen criaturas en zonas seguras
If Hechizos(H).NumNpc <> 0 And MapInfo(UserList(UserIndex).Pos.Map).Pk = False Or MapData(UserList(UserIndex).Pos.Map, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).trigger = eTrigger.ZONASEGURA Then
...
UserList(UserIndex).Pos.Map = CastilloEste Or UserList(UserIndex).Pos.Map = CastilloOeste Or UserList(UserIndex).Pos.Map = mapDuelo Then
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRE33")
    Exit Sub
End If
...
'If Hechizos(H).NumNpc = 0 And UserList(UserIndex).Pet.Nombre = "" Then Exit Sub

If Hechizos(H).NumNpc = 0 Then
    If UserList(UserIndex).Pet.MinHP <= 0 Then
        Call SendData(ToIndex, UserIndex, 0, "PRE46")
        Exit Sub
    End If
    If UserList(UserIndex).Pet.Invocada = False Then
    ...
    Else
        ...
        Call MuereNpc(UserList(UserIndex).Pet.Index, 0)
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB24")
        Exit Sub
    End If
Else
    NumNpc = Hechizos(H).NumNpc
End If
...
For j = 1 To Hechizos(H).Cant
    ...
    If UserList(UserIndex).NroMacotas < MAXMASCOTAS Then
        ind = SpawnNpc(NumNpc, TargetPos, True, False)
        If ind > 0 Then
            UserList(UserIndex).NroMacotas = UserList(UserIndex).NroMacotas + 1
             ...
             If Hechizos(H).NumNpc = 0 Then
                With UserList(UserIndex).Pet
                    .Index = ind
                    .Invocada = True
                    Npclist(ind).name = .Nombre
                    Npclist(ind).Stats.def = .Defensa
                    Npclist(ind).Stats.MaxHIT = .MaxHIT
                    Npclist(ind).Stats.MinHIT = .MinHIT
                    Npclist(ind).Stats.MinHP = .MinHP
                    Npclist(ind).Stats.MaxHP = .MaxHP
                End With
                Npclist(ind).Contadores.TiempoExistencia = 0
                Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB25")
            Else
...
If b Then
    If Hechizos(uh).ManaRequerido > 0 Then Call SubirSkill(UserIndex, Magia)
...
#If SeguridadAlkon Then
    ...
    Else
#End If
        Crypted = "NOVER" & UserList(TU).Char.CharIndex & ",1"
        Call SendData(SendTarget.ToMap, 0, UserList(TU).Pos.Map, "ç" & Encrypt_PRO(Crypted, CRYPTO_PASS)) '"NOVER" & UserList(TU).Char.CharIndex & ",1")
        'If Guilds(UserList(TU).GuildIndex).GuildName <> "" Then Call SendData(SendTarget.toguildmembers, TU, UserList(TU).Pos.Map, "PUDVE" & UserList(TU).Char.CharIndex & ",1")
#If SeguridadAlkon Then
    End If
#End If
    Call InfoHechizo(UserIndex)
...
#Else
        Call SendData(SendTarget.ToIndex, TU, 0, "ç" & Encrypt_PRO("CEGU", CRYPTO_PASS))
#End If
        Call InfoHechizo(UserIndex)
        b = True
End If
...
#If SeguridadAlkon Then
        ...
        Else
#End If
            Call SendData(SendTarget.ToIndex, TU, 0, "ç" & Encrypt_PRO("DUMB", CRYPTO_PASS))
#If SeguridadAlkon Then
        End If
#End If
        Call InfoHechizo(UserIndex)
...
If Npclist(NpcIndex).NPCtype = eNPCType.ReydelCastillo Then
    If UserList(UserIndex).GuildIndex = 0 Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRE17")
        Exit Sub
    Else
        castOeste = GetVar(App.Path & "\Dat\Castillos.dat", "INIT", "Oeste")
        castEste = GetVar(App.Path & "\Dat\Castillos.dat", "INIT", "Este")
        castNorte = GetVar(App.Path & "\Dat\Castillos.dat", "INIT", "Norte")
        castSur = GetVar(App.Path & "\Dat\Castillos.dat", "INIT", "Sur")
        
        If UserList(UserIndex).Pos.Map = CastilloOeste And castOeste = Guilds(UserList(UserIndex).GuildIndex).GuildName Or _
            UserList(UserIndex).Pos.Map = CastilloEste And castEste = Guilds(UserList(UserIndex).GuildIndex).GuildName Or _
            UserList(UserIndex).Pos.Map = CastilloNorte And castNorte = Guilds(UserList(UserIndex).GuildIndex).GuildName Or _
            UserList(UserIndex).Pos.Map = CastilloSur And castSur = Guilds(UserList(UserIndex).GuildIndex).GuildName Then
            
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRE19")
            Exit Sub
        
        Else
            Call AtacandoCastillo(UserIndex, NpcIndex)
        End If
    End If
End If
    ...
    Npclist(NpcIndex).Stats.MinHP = Npclist(NpcIndex).Stats.MinHP - daño
    SendData SendTarget.ToIndex, UserIndex, 0, "PRB27," & daño
...
'End If
    ...
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRB28," & daño & "," & UserList(tempChr).name)
    Call SendData(SendTarget.ToIndex, tempChr, 0, "PRB29," & UserList(UserIndex).name & "," & daño)
