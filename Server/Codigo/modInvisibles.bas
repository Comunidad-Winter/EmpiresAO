...
#If SeguridadAlkon Then
    ...
    Else
#End If
        Crypted = "NOVER" & UserList(UserIndex).Char.CharIndex & "," & IIf(estado, 1, 0)
        Call SendData(SendTarget.ToMap, 0, UserList(UserIndex).Pos.Map, "ç" & Encrypt_PRO(Crypted, CRYPTO_PASS)) '"NOVER" & UserList(UserIndex).Char.CharIndex & "," & IIf(estado, 1, 0))
       ' If Guilds(UserList(UserIndex).GuildIndex).GuildName <> "" Then Call SendData(SendTarget.toguildmembers, UserIndex, UserList(UserIndex).Pos.Map, "PUDVE" & UserList(UserIndex).Char.CharIndex & "," & IIf(estado, 1, 0))
#If SeguridadAlkon Then
    End If
#End If
