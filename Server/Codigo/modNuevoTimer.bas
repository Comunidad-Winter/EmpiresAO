...
Public Function IntervaloPotear(ByVal UserIndex As Integer, Optional ByVal Actualizar As Boolean = True) As Boolean
Dim TActual As Long

TActual = GetTickCount() And &H7FFFFFFF

If TActual - UserList(UserIndex).Counters.TimerPuedePotear >= 9 * IntervaloUserPuedePotear Then
    If Actualizar Then UserList(UserIndex).Counters.TimerPuedePotear = TActual
    IntervaloPotear = True
Else
    IntervaloPotear = False
End If
End Function
